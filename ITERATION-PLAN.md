...
### D2 · P1 · 跨文档哈希串档（`load()` 未重置 `_pdfHashPromise`）
...
### D4 · P1 · `_computeHashFromUrl` 缺陈旧性校验
...
### D6 · P2 · `toFlatJSON()` 不含 hash，与 `toJSON()` 不一致
...
### D8 · P2 · `importJSON()` 缺 try/finally，`_currentUserId` 泄漏
...
| **v4.9.0** | 稳定性 | H1 加载生命周期统一（消灭 D2/D3/D4/D5）+ H2 内存 + H3 可观测性 | 中 |
| **v4.9.1** | 一致性打磨 | D6 / D7 / D8 / D9（加选项，不改默认）/ D11 | 小 |
