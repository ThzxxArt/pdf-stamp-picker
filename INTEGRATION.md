...
- **旧浏览器（Edge 90 / Chrome 97 及更旧）**：pdf.js 3.11 依赖 `Array.at`（92+）/`structuredClone`（98+）等。默认 `compatCheck: true` 显示**升级提示**（避免晦涩报错）；`compatCheck: false` 走 polyfill + script 加载 worker 到主线程 + fake worker 兜底（尽力运行）。**v4.8.15+ 默认提示升级**
