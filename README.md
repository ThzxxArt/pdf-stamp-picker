...
3. 🛡️ **章大小固定**（`stampSize` 决定，默认 **120 pt ≈ 4.2 cm**，PDF 物理单位）——**不可缩放，也不随窗口宽度变化**：无缩放手柄、Ctrl+滚轮/双指均不缩放。窄屏上章显示得更小（显示尺寸 = `stampSize × 当前缩放`），但**落库与导出的物理尺寸恒定**
...
| `stampSize` | `120` | 签章图基准宽度 **pt（PDF 物理单位）**，默认 120 pt ≈ 4.2 cm（接近标准公章直径）。章固定大小、不可运行时缩放，且**不随窗口宽度/显示缩放变化** |
...
| **签章点** | `addStamp({x,y,width,height,page,userId,note})` | 程序化添加签章点（PDF 坐标，`x/y` = 左上角）→ 返回 stamp。**尺寸缺省时按模式补齐**：`point` 模式 → 0×0 锚点；其余模式 → `stampSize`（pt，默认 120）。与"在画布上点一下"得到的**物理尺寸完全一致** —— 两条路径在任何窗口宽度/缩放下都相等（v4.9.4 起；显式传 `0,0` 仍为锚点） |
...
- **签章尺寸的单位（v4.9.4 起明确）**：`stampSize`（默认 120）是 **PDF pt 物理尺寸**，与屏幕像素无关 —— 页面被缩放或容器很窄时，落库/导出的章尺寸恒定（120 pt ≈ 4.2 cm，接近标准公章直径 4.2 cm）。屏幕显示尺寸 = `stampSize × 当前缩放`，由库在点击放置时自动换算；`addStamp()` 的缺省尺寸与点击放置**在任何缩放下都相等**。
...
- ⚠️ **行为变更（v4.9.4）· 签章尺寸单位收口**：v4.9.3 及以前 `stampSize` 的单位是**混淆的** —— `addStamp()` 把同一个数当 pt 直接落库，而画布点击放置路径把它当**屏幕 px** 使用（显示矩形不乘缩放）。结果是**同一次"点一下放章"在两处得到不同的物理尺寸，只有缩放恰为 1 时两者才相等**：容器 360 px 时点一下会产出 **1428 pt（页宽 240%）** 的章，且窗口越窄章越大，签名结果取决于操作者窗口宽度，窄屏下无 API 可纠正。v4.9.4 起统一为 **pt（物理尺寸恒定）**：点击路径的显示矩形 = `stampSize × 当前缩放`，两路在任何缩放下完全相等。**升级影响**：窄屏/小容器下点击放置的章，导出的物理尺寸会比旧版**变小**（变回正确的 `stampSize`）；若此前靠"窄屏放大效应"得到过特定尺寸的章，请改用 `stampSize` 显式指定（详见 `demo/addstamp-size-test.html` ⑥）。
